...
func (pt *PodsTracker) processPodUpdates(ctx context.Context, pod *v1.Pod) bool {
	...
	if err != nil {
		log.G(ctx).WithError(err).Warnf("cannot fetch [provider] events for pod %s in namespace %s", pod.Name, pod.Namespace)
	}
	...
	if err == nil && podStatusFromProvider != nil {
		if !IsDesiredPodCondition(podStatusFromProvider, pod) {
